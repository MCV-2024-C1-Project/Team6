...
class HellingerKernelSimilarity(Measurement):
    ...
    def __call__(self, im1_hist : list, im2_hist : list):
        ...
        for c in range(len(im1_hist)):
            dist.append(np.sum(np.sqrt(im1_hist[c] * im2_hist[c])))
        dist = np.array(dist).mean()
        return -dist #its a similiratiy (the greater the better) so if we wish to treat it as a distance we should multiply by -1
...
class HistogramIntersectionSimilarity(Measurement):
    ...
    def __call__(self, im1_hist : list, im2_hist : list):
        ...
        return -sum ([ min(x,y) for x,y in zip(im1_hist, im2_hist) ])#its a similiratiy (the greater the better) so if we wish to treat it as a distance we should multiply by -1